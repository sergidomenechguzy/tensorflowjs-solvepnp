...
const Ps = tf.tensor2d([[0.4, -1, 2, 1], [-0.5, 0.5, 2, 1], [1.5, -0.5, 4, 1]]);
...
const ps = tf.tensor2d([[1290, 1210, 1], [540, 2460, 1], [1165, 1210, 1]]);
...
const solvePNPNoRMultiple = Ps => {
  const [p0, p1, p2] = tf.unstack(Ps);
  return tf.stack([solvePNPNoR(p0), solvePNPNoR(p1), solvePNPNoR(p2)]);
};
...
const solvePNPOneRMultiple = Ps => {
  const [p0, p1, p2] = tf.unstack(Ps);
  return tf.stack([solvePNPOneR(p0), solvePNPOneR(p1), solvePNPOneR(p2)]);
};
...
const trainNetOneR = () => {
  ...
  for (let i = 0; i < 3000; i++) {
    const cost = optimizer.minimize(
      () => tf.losses.meanSquaredError(ps, solvePNPOneRMultiple(Ps)),
      true
    );
    cost.print();
  }
